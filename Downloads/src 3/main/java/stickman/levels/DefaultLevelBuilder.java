...
public class DefaultLevelBuilder extends LevelBuilder {
    public static int lives = 3;//addition
    Slime.Colour color;
    SlimeStrategy slimeStrategy;
    ...
    protected void buildSlimes() {
        ...
        for (int i = 0; i < slimes.size(); i++) {
            ...
            x = ((Double) group.get("x"));
            y = ((Double) group.get("y"));
            String colorString  = ((String) group.get("color"));
            if(colorString.equalsIgnoreCase("blue")) {
                color = Slime.Colour.BLUE;
                slimeStrategy = new BlueSlimeStrategy();
            }else if(colorString.equalsIgnoreCase("green")) {
                color = Slime.Colour.GREEN;
                slimeStrategy = new GreenSlimeStrategy();
            }else if(colorString.equalsIgnoreCase("purple")) {
                color = Slime.Colour.PURPLE;
                slimeStrategy = new PurpleSlimeStrategy();
            }else if(colorString.equalsIgnoreCase("red")) {
                color = Slime.Colour.RED;
                slimeStrategy = new RedSlimeStrategy();
            }else if(colorString.equalsIgnoreCase("yellow")) {
                color = Slime.Colour.YELLOW;
                slimeStrategy = new YellowSlimeStrategy();
            }
            Entity slime = new Slime(x, y, color, slimeStrategy);
            entities.add(slime);
        }
    }
...
//    }
    ...
    @Override
    protected void buildTargetTime(){
        JSONObject timer = (JSONObject)configuration.get("timer");
        double target = ((Double)timer.get("target"));
        int targetTime = (int) target;
